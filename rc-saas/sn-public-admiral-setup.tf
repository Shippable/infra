...
# varsha: x86/64 Centos 7 Instances
 resource "aws_instance" "admiral_varsha_centos7" {
   ami = "${var.ami_us_east_1_centos7}"
   availability_zone = "${var.avl-zone}"
   instance_type = "${var.in_type_ms_x}"
   key_name = "${var.aws_key_name}"
   ...
   tags = {
     Name = "admiral_varsha_centos7_${count.index}_${var.install_version}"
   }
 }

 output "admiral_varsha_centos7" {
   value = "${formatlist("instance %v has private ip %v", aws_instance.admiral_varsha_centos7.*.id, aws_instance.admiral_varsha_centos7.*.private_ip)}"
 }
